...
public class HighlightRenderer {
    ...
    private static final RenderType FILL_RENDER_TYPE_HIDE = RenderType.create(
            "highlight_fill_hide",
            DefaultVertexFormat.POSITION_COLOR,
            VertexFormat.Mode.QUADS,
            256, false, true,
    ...
    private static void renderFill(PoseStack poseStack, MultiBufferSource.BufferSource bufferSource, AABB aabb, float[] color, float alpha, DepthMode depthMode) {
        RenderType renderType = depthMode == DepthMode.IGNORE ? FILL_RENDER_TYPE : FILL_RENDER_TYPE_HIDE;
    ...
    private static void renderBlock(RenderLevelStageEvent event, HighlightHandle handle, BlockPos pos) {
        ...
        renderFill(poseStack, bufferSource, new AABB(pos).inflate(0.002), handle.getFillColor(), alpha, handle.getDepthMode());
    ...
    private static void renderEntity(RenderLevelStageEvent event, HighlightHandle handle, Entity entity) {
        ...
        renderFill(poseStack, bufferSource, interpolatedBox.inflate(0.002), handle.getFillColor(), alpha, handle.getDepthMode());
    ...
    private static void renderAABB(RenderLevelStageEvent event, HighlightHandle handle, AABB aabb) {
        ...
        renderFill(poseStack, bufferSource, aabb.inflate(0.002), handle.getFillColor(), alpha, handle.getDepthMode());
    ...
    public static void onWorldLoad(LevelEvent.Load event) {
        ...
        // HIDE - должен прятаться за блоками
        HighlightAPI.create(Lifetime.ETERNAL)
                ...
                .setDepthMode(DepthMode.HIDE)
                .register();


        HighlightAPI.create(Lifetime.ETERNAL)
                .setTarget(new BlockPos(3, 64, 0))
                .setOutlineColor(0.0f, 1.0f, 0.0f, 1.0f)
                .setFillColor(0.0f, 1.0f, 0.0f, 0.3f)
